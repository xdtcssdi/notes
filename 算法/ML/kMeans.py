...
def kmeans(data, n, m, k, plt):
    # 获取k个随机数, 
    # 乘以数据集大小——>数据集中随机的4个点
    rarray = np.floor(np.random.random(size=k) * n).astype(int)
    print('数据集中随机索引', rarray) #shape=(k,)
    # 随机取数据集中的4个点作为初始中心点(老大)
    center = data[rarray]
    # 测试比较偏、比较集中的点，效果依然完美，测试需要删除以上代码
    # center = np.array([[4.6,-2.5],[4.4,-1.7],[4.3,-0.7],[4.8,-1.1]])
    # 1行80列的0数组，标记每个样本所属的类(k[i])
    cls = np.zeros([n], np.int) #n个数据的标签，shape=(n, )
    print('初始center=\n', center)
    run = True
    time = 0
    while run:
        time = time + 1
        for i in range(n): # 批量求对每一个点求到中心点的距离
            # 求差
            tmp = data[i] - center
            # 求平方
            tmp = np.square(tmp)
...
if __name__ == "__main__":
    csv = pd.read_csv("/Users/oswin/Desktop/KmeansData.txt")
    # 打印原始数据
